...
public:
    ...
    void givePartialOutput(HTMLBuilder& log, std::string filename)
    {
        ...
        HTMLencode(partialOutputString);
    ...
    std::string getResult(HTMLBuilder& log, std::string executablename, std::string args, std::string inputfilename, std::string outputfilename, int timelimit, SIZE_T memlimit, int pnum)
    {
    ...
    bool runtest(HTMLBuilder& log, std::string executablename, std::string args, std::string inputfilename, std::string outputfilename, std::string answerfilename, int timelimit, SIZE_T memlimit, int pnum, mode runmode)
    {
        switch(runmode)
        {
        case mode::NORMAL:
    ...
    bool runExe(HTMLBuilder& log, std::string executable, std::string in, std::string out, std::string ans, int timelimit, SIZE_T memlimit, int pnum, mode runmode)
    {
        return runtest(log, executable, "", in, out, ans, timelimit, memlimit, pnum, runmode);
    }
    bool runCpp(HTMLBuilder& log, std::string cppFile, std::string in, std::string out, std::string ans, int timelimit, SIZE_T memlimit, int pnum, mode runmode)
    {
        return runtest(log, cppFile.substr(0, cppFile.rfind('.'))+std::string(".exe"), "", in, out, ans, timelimit, memlimit, pnum, runmode);
    }
    bool runJava(HTMLBuilder& log, std::string javaDir, std::string javaFile, std::string in, std::string out, std::string ans, int timelimit, SIZE_T memlimit, int pnum, mode runmode)
    {
        return runtest(log, "C:\\Windows\\System32\\cmd.exe", "/c java -cp "+javaDir+" "+javaFile, in, out, ans, timelimit, memlimit, pnum, runmode);
    }
    bool runPython(HTMLBuilder& log, std::string pythonVersion, std::string pythonFile, std::string in, std::string out, std::string ans, int timelimit, SIZE_T memlimit, int pnum, mode runmode)
    {
    ...
    int run(std::string program, std::string problemName, std::string problemCode, std::string timestamp)
    {
